...
- **Ollama** (in-stack service; host port 11737): Local LLM used by `distill_sessions.py` and `behavioral_pass.py` for session distillation. No API key required. Recommended model: `qwen2.5:7b`.
...
Uses the in-stack `ollama` compose service by default (`http://ollama:11434/v1`; host-side published as `localhost:11737`). Override via `OLLAMA_URL`. Uses `DISTILL_MODEL` env var (default: `qwen2.5:7b`).
...
| `OLLAMA_URL` | `http://ollama:11434/v1` | Ollama endpoint for distillation — the in-stack `ollama` compose service (host-side default: `http://localhost:11737/v1`) |
